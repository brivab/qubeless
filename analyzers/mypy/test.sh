...
# Test script for Mypy analyzer
# This script builds the Docker image and runs it against the demo project

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
DEMO_DIR="$PROJECT_ROOT/examples/python-mypy-demo"
OUT_DIR="$SCRIPT_DIR/test-output"

IMAGE_NAME="mypy-analyzer"
IMAGE_TAG="latest"

echo "=== Mypy Analyzer Test ==="
echo ""
...
if [ -f "$REPORT_JSON" ]; then
  ...
  if [ "$ISSUES_COUNT" -ge 2 ]; then
    echo "✓ At least 2 issues detected (as expected)"
  else
    echo "⚠️  Expected at least 2 type errors from demo project, found $ISSUES_COUNT"
  fi
fi
...
with open(report_path) as f:
    ...
    for i, issue in enumerate(data['issues'][:5], 1):
        print(f"  {i}. [{issue['severity']}] {issue['type']} - {issue['ruleKey']}")
        print(f"     {issue['filePath']}:{issue['line']}:{issue['column']}")
